...
\begin{frame}{Exemple~: Statistiques de vente}
	...
	\begin{pseudo}
		...
		\Read numéroProduit
		\Empty
		\While{numéroProduit > 0 ET numéroProduit $\leq$ nbArticles}
		\Empty
			\Write "Introduisez la quantité vendue~:"
			\Read quantité
			\Empty
...
\begin{frame}{Tableau statique vs tableau dynamique~: exemple dynamique}
	Pour déclarer un tableau dynamique, on sépare 
	\begin{itemize}
		\item
		la déclaration du tableau	(où la taille n’est pas donnée) 
		\item
		de sa création effective~(où on donne la taille)
	\end{itemize}
